...
cc = subset(cc,cc$nee < 12 & cc$nee > -14)
...
ggplot(data = cf)+theme_bw()+
  ...
  geom_point(aes(date,FCH4,col='Real'),alpha=0.5)+
...
ggplot(data = cf)+theme_bw()+
  ...
  geom_point(aes(date,FCH4,col='Real'),alpha=0.2)
```
...
yk2ch4 = ggplot(data = test.data.ch4,aes(rfch4,FCH4))+theme_bw()+
  geom_hline(yintercept = 0,lty=2)+
  geom_vline(xintercept = 0,lty=2)+
  geom_point(alpha=0.2)+
  scale_fill_viridis_c()+
...
yk2ch4
summary(lm(test.data.ch4$FCH4 ~ test.data.ch4$rfch4))
```
...
save off the data
```{r,error=FALSE,warning=FALSE}
write.csv(x = cf,file = "./council_2017_2022_gf.csv",row.names = F)
```
